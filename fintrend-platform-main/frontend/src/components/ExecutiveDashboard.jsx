...
export default function ExecutiveDashboard() {
  ...
  const [riskCompliance, setRiskCompliance] = useState([
    { metric: 'SIP Stoppage Ratio', value: '1.24%', status: 'low', trend: 'stable' },
    { metric: 'Large Redemptions Alert', value: '3', status: 'medium', trend: 'up' },
    { metric: 'Compliance Exceptions', value: '0', status: 'low', trend: 'stable' },
    { metric: 'Concentration Risk', value: 'Moderate', status: 'medium', trend: 'up' },
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // 6. Update Risk & Compliance Monitor
      setRiskCompliance(prev => {
        const newSipStoppage = parseFloat(prev[0].value) + rnd(-0.02, 0.03);
        const redemptions = parseInt(prev[1].value) + (Math.random() > 0.8 ? 1 : (Math.random() > 0.8 ? -1 : 0));
        return [
          { ...prev[0], value: `${Math.max(0.1, newSipStoppage).toFixed(2)}%`, trend: newSipStoppage > parseFloat(prev[0].value) ? 'up' : 'down' },
          { ...prev[1], value: Math.max(0, redemptions).toString(), status: redemptions > 5 ? 'high' : (redemptions > 2 ? 'medium' : 'low'), trend: redemptions > parseInt(prev[1].value) ? 'up' : (redemptions < parseInt(prev[1].value) ? 'down' : 'stable') },
          { ...prev[2], value: (Math.random() > 0.95 ? parseInt(prev[2].value) + 1 : prev[2].value).toString(), status: parseInt(prev[2].value) > 0 ? 'medium' : 'low', trend: 'stable' },
          { ...prev[3], status: Math.random() > 0.9 ? 'high' : prev[3].status, trend: Math.random() > 0.5 ? 'up' : 'stable' },
          { ...prev[4], status: 'low', trend: 'stable' }
        ];
      });
